...
不计费的模型不受日/周 USD 限额限制：免费请求不会消耗预算，所以到达上限也不应拦它。

满足以下任一条件即视为免费：价表里有该模型且匹配到的规则**所有费率都为 `0`**（输入、输出、缓存读、缓存写、每次请求）；或者价表里**完全没有该模型**（未收录的模型本就不计费）。注意：输入免费但输出收费的规则**不算**免费。

RPM 仍然生效：它限制请求速率，不是花费。空价表不会让所有模型变免费 —— 没有任何规则时运维尚未表达意图，门控保持原行为。

计价使用与计费完全相同的匹配器（`MatchPlusPrice`），所以门控与实际账单永远不会对「这次请求是否花钱」产生分歧。
